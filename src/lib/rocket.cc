...
public:
    static void decode(istream& in, iostream& Dst) {
        ...
        auto const  OutSize = static_cast<std::streamsize>(BigEndian::Read2(in));
        auto const  InSize  = static_cast<std::streamsize>(BigEndian::Read2(in) + 4);
        RockIStream src(in);

        while (in.good() && in.tellg() < InSize && Dst.tellp() < OutSize) {
            if (src.descbit() != 0U) {
                // Symbolwise match.
