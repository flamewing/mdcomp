...
public:
	static void decode(std::istream &in, std::iostream &Dst, unsigned short stream_end) {
		RockIStream src(in);
...
bool rocket::decode(istream &Src, iostream &Dst, streampos Location) {
	Src.seekg(Location);
...
bool rocket::encode(istream &Src, ostream &Dst) {
	...
	rocket_internal::encode(Dst, ptr, BSize);

	// Fill in header
	size_t compressed_size = Dst.tellp();
	Dst.seekp(0);
	BigEndian::Write2(Dst, ISize);					// Size of decompressed file
	BigEndian::Write2(Dst, compressed_size - 4);	// Size of compressed file
