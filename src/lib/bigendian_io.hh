...
namespace detail {
	...
	struct has_iterator_tag : boost::mpl::bool_<
		std::is_same<get_iterator_tag_t<Cont>, Tag>::value> {
	};

	template <typename Ptr>
	struct is_byte_pointer : boost::mpl::bool_<
			conjunction<std::is_pointer<Ptr>,
			            is_any_of<typename std::iterator_traits<Ptr>::value_type,
			                      char, unsigned char, signed char>>::value> {
	};

	template <typename T>
	struct is_void_pointer : boost::mpl::bool_<
			conjunction<std::is_pointer<T>,
			            std::is_same<std::remove_cv_t<typename std::pointer_traits<T>::element_type>, void>>::value> {
	};

	template <typename T>
	struct is_pointer_like : boost::mpl::bool_<
			conjunction<negation<is_void_pointer<T>>,
			            has_iterator_category<get_iterator_traits_t<T>>
			>::value> {
	};

	template <typename T>
	struct is_contiguous_container : boost::mpl::bool_<
			conjunction<std::is_class<T>, has_value_type<T>, has_iterator<T>, has_size_type<T>,
			            has_reference<T>, has_push_back<T>, has_size<T>, has_resize<T>, has_data<T>,
	...
	private:
		...
		// I am splitting these cases because MSVC compiler does not understand memmove
		// (which is called my std::copy which is called by std::copy_n).
		...
		template <typename Iter, typename T>
		static inline void ReadInternal(Iter& in, T& val, std::forward_iterator_tag) noexcept  {
			ReadBase(in, val);
			std::advance(in, sizeof(T));
		}

		template <typename Iter, typename T>
		static inline void ReadInternal(Iter& in, T& val, std::input_iterator_tag) noexcept  {
			ReadBase(in, val);
		}
		...
		template <typename Ptr, typename T>
		static inline void WriteInternal(Ptr& out, T val, std::forward_iterator_tag) noexcept  {
			WriteBase(out, val);
			std::advance(out, sizeof(T));
		}

		template <typename Iter, typename T>
		static inline void WriteInternal(Iter& out, T val, std::output_iterator_tag) noexcept  {
			WriteBase(out, val);
		}
	...
	public:
		...
		static inline auto Read(Iter& in, T& val) noexcept
			...
			ReadInternal(in, val, typename std::iterator_traits<Iter>::iterator_category());
		}
		...
		static inline auto Write(Cont& out, T val) noexcept
					-> std::enable_if_t<detail::is_contiguous_container<Cont>::value, void> {
			auto sz = out.size();
		...
		static inline auto Write(Iter& out, T val) noexcept
					-> std::enable_if_t<detail::is_pointer_like<Iter>::value, void> {
			WriteInternal(out, val, typename std::iterator_traits<Iter>::iterator_category());
		}
	};
	...
	struct EndianBase {
		...
		static inline auto ReadN(Src& in) noexcept -> detail::select_unsigned_t<Size> {
		...
		template <typename Src, typename T>
		static inline auto Read(Src& in, T& val) noexcept -> std::enable_if_t<std::is_unsigned<T>::value, void> {
			Base::Read(in, val);
		}
		...
		template <typename Dst, size_t Size, typename Uint_t = detail::select_unsigned_t<Size>>
		static inline void WriteN(Dst& out, Uint_t val) noexcept {
			Base::Write(out, val);
		}

		template <typename Dst, typename T>
		static inline auto Write(Dst& out, T val) noexcept -> std::enable_if_t<std::is_unsigned<T>::value, void> {
			Base::Write(out, val);
		}
	};
}
