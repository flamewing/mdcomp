...
struct comper_adaptor {
    ...
    using sliding_window_t    = lzss::sliding_window<comper_adaptor>;
    using adj_list_node       = lzss::adj_list_node<comper_adaptor>;
    using adj_list            = std::list<adj_list_node>;
    using istream_t           = lzss::istream<comper_adaptor>;
    using ostream_t           = lzss::ostream<comper_adaptor>;
    ...
    constexpr static bool decode_edge(
        ...
        if (source.descriptor_bit() == 0U) {
            // Symbolwise match.
            return lzss::symbolwise_match<comper_adaptor>(
        ...
        if (size_t const value = source.get_byte(); value != 0) {
            ...
            return lzss::dictionary_match<comper_adaptor>(
                    nodes, output_size, source.tellg(), distance, length,
                    edge_type::dictionary);
        }
        return lzss::terminate<comper_adaptor>(nodes, output_size);
    }
    ...
    constexpr static size_t output_edge(std::iostream& dest, adj_list_node const& edge) {
        ...
        case dictionary: {
            ...
            lzss::copy<comper_adaptor>(dest, distance, length);
            break;
        }
        case terminator:
            break;
...
static_assert(
        lzss::adaptor_t<comper_adaptor>,
        "comper_adaptor does not satisfy lzss::adaptor_t requirements");

struct comper_internal {
    static void decode(std::istream& input, std::iostream& dest) {
        using adaptor_t     = comper_adaptor;
    ...
    static void encode(std::ostream& dest, std::span<char const> data) {
        lzss::encode(dest, data, comper_adaptor{});
    }
};
...
bool comper::encode(std::ostream& dest, std::span<char const> data) {
